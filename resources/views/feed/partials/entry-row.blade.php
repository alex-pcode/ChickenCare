    ...
    <td class="data-table__cell data-table__actions">
        @if($feed->isActive())
            <button type="button" class="btn btn--sm btn--secondary"
                ...
                hx-swap="outerHTML"
                aria-label="{{ __('feed.actions.mark_depleted_aria') }}">
                {{ __('feed.actions.mark_depleted') }}
            </button>
        @endif
        <button type="button" class="btn btn--sm btn--secondary"
            ...
            hx-target="#feed-{{ $feed->id }}"
            hx-swap="outerHTML"
            aria-label="{{ __('feed.actions.edit_aria') }}">
            {{ __('feed.actions.edit') }}
        </button>
        <button type="button"
            class="data-table__delete-btn"
            hx-get="{{ route('app.feed.delete-confirm', $feed) }}"
            hx-target="#modal-container"
            aria-label="{{ __('feed.actions.delete_aria') }}">
